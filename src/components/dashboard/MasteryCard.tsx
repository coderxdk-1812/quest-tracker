...
/** Per-subject mastery — competence framing ("you're getting better at X"). */
    ...
    <div className="glass-card p-5">
      ...
      <div className="space-y-3">
        {mastery.slice(0, 6).map((m, i) => (
          <motion.div key={m.subject} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className="h-1.5 bg-muted rounded-full overflow-hidden">
              <motion.div className="h-full xp-gradient" initial={{ width: 0 }}
                animate={{ width: `${m.pct}%` }} transition={{ duration: 0.6 }} />
            </div>
          </motion.div>
        ))}
      </div>
