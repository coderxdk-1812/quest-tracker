...
export function buildShareText(s: ShareStats): string {
  return (
    `🏆 ${s.displayName} — ${s.rankTitle} (Level ${s.level}) on Level Up Quest\n` +
...
export function buildShareSvg(s: ShareStats): string {
  ...
  <text x="40" y="58" fill="#6b7280" font-size="15" letter-spacing="2" font-family="Arial, sans-serif">LEVEL UP QUEST</text>
