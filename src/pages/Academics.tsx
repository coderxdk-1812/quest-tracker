...
import { Target, Plus, Trash2, CalendarClock, Flame, BarChart3, Brain } from 'lucide-react';
import { useGame } from '@/context/GameContext';
import { useAcademics } from '@/hooks/useAcademics';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import {
  daysUntil, examUrgency, countdownLabel, goalProgress, effortBySubject,
...
export default function Academics() {
  ...
  const effort = useMemo(() => effortBySubject(state.tasks), [state.tasks]);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <section className="glass-card p-5">
        ...
        <p className="text-xs text-muted-foreground mb-3">
          What you've actually put in — effort is what drives results. ({state.focusSessionsCompleted} focus sessions all-time)
        </p>
        {effort.length === 0 ? (
          <p className="text-sm text-muted-foreground py-2">Add subjects to your tasks to see effort here.</p>
        ) : (
